...
import { IconCheck, IconLock, IconMail, IconPhone, IconPin, IconWhats } from "./Icons";
...
export default function Contact() {
  ...
  const onSubmit = (e: FormEvent) => {
    ...
    const lines = [
      "Olá, Bruna! Gostaria de um orçamento/contato sobre perícia trabalhista.",
      "",
      `*Nome:* ${form.nome.trim()}`,
      `*E-mail:* ${form.email.trim()}`,
      form.whats.trim() ? `*WhatsApp:* ${form.whats.trim()}` : "",
      form.msg.trim() ? `*Mensagem:* ${form.msg.trim()}` : "",
    ].filter(Boolean).join("\n");

    const waUrl = `https://wa.me/553185856534?text=${encodeURIComponent(lines)}`;
      ...
      <div className="container-x grid gap-12 lg:grid-cols-12 lg:gap-10">
        ...
        <Reveal delay={120} className="lg:col-span-5">
          <form onSubmit={onSubmit} className="flex flex-col gap-3" noValidate={false}>
            ...
            <input id="whats" type="tel" value={form.whats} onChange={set("whats")} className="field" placeholder="WhatsApp (opcional)" />
            <label className="sr-only" htmlFor="msg">Como posso ajudar?</label>
            <textarea id="msg" rows={4} value={form.msg} onChange={set("msg")} className="field resize-none" placeholder="Como posso ajudar? Descreva brevemente o seu caso..." />
            ...
            >
              <div className="min-h-0 overflow-hidden">
                <p className="mt-3 flex items-center gap-3 border border-gold-500/50 bg-gold-400/10 px-4 py-3 text-[12px] text-inktext">
                  ...
                  Abrindo WhatsApp com os dados preenchidos...
                </p>
              </div>
            </div>
          </form>
